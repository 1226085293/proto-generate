...
(function (main) {
                ...
                fs.readFile(`${storage_o.cache_a.output_path_s}/${storage_o.cache_a.output_name_s}.js`, 'utf-8', (err_a, data_s) => {
                    ...
                    fs.writeFile(`${storage_o.cache_a.output_path_s}/${storage_o.cache_a.output_name_s}.js`, data_s, (err, data) => {
                        ...
                        if (storage_o.cache_a.reduce_code_b) {
                            child_process.exec(`node uglifyjs ${storage_o.cache_a.output_path_s}/${storage_o.cache_a.output_name_s}.js -m -o ${storage_o.cache_a.output_path_s}/${storage_o.cache_a.output_name_s}.js`, {
    ...
    main.messages = {
        ...
        /**文件改变 */
        "asset-db:asset-changed"(event1, event2_a) {
            let path = Editor.assetdb.uuidToUrl(event2_a.uuid);
            update_proto(path.substring(path.indexOf("assets"), path.length));
        },
        /**文件删除 */
        "asset-db:assets-deleted"(event1, event2_a) {
            let path = event2_a[0].url;
            update_proto(path.substring(path.indexOf("assets"), path.length));
        },
    };
